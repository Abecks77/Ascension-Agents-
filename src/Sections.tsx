...
export const WhoThisIsFor = () => {
  ...
  const tabs = [
    { 
      ...
      title: "Scale Saturation Detected",
      data: "Revenue plateauing due to extreme operational friction.",
      stats: [
        { label: "Efficiency Floor", value: "94%", color: "text-red-500" },
        { label: "Growth Drag", value: "5.2x", color: "text-orange-600" },
        { label: "System Load", value: "CRITICAL", color: "text-red-600" }
      ],
      details: [
        "Headcount growth has reached diminishing returns (1.2x cost/rev)",
        "Key personnel spend >60% of time on 'keep-the-lights-on' tasks",
        "New product velocity has dropped by 40% due to ops baggage"
      ]
    },
    { 
      label: "The Leak", 
      ...
      data: "Significant capital loss identified in the sales pipeline.",
      stats: [
        { label: "Lost Opportunity", value: "$42k/mo", color: "text-red-600" },
        { label: "Response Delay", value: "6.2 hrs", color: "text-orange-600" },
        { label: "Lead Drop-off", value: "34%", color: "text-slate-900" }
      ],
      details: [
        "Inbound leads are ghosting due to inconsistent follow-up speed",
        "Zero automated nurturing is happening after the 48-hour mark",
        "CRM data integrity is below 60% due to manual entry errors"
      ]
    },
    { 
      label: "The Solution", 
      icon: <Cpu className="w-4 h-4" />, 
      title: "Agentic Infrastructure Blueprint",
      data: "Deploying autonomous systems for immediate operational lift.",
      stats: [
        { label: "Time Recovered", value: "25 hrs/wk", color: "text-blue-600" },
        { label: "Scale Factor", value: "10x", color: "text-emerald-600" },
        { label: "ROI Target", value: "480%", color: "text-blue-600" }
      ],
      details: [
        "Automated lead enrichment & routing (0.2s latency)",
        "Self-managing follow-up sequences based on intent signals",
        "Live dashboard reporting generated by analytics agents"
      ]
    }
  ];
    ...
    <section id="who-this-is-for" className="py-24 relative overflow-hidden w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 w-full overflow-hidden">
        <div className="flex flex-col items-center gap-12 lg:gap-16 w-full">
          ...
          <FadeIn delay={0.2} className="w-full max-w-5xl mx-auto">
            <div className="bg-white rounded-[2.5rem] border border-gray-200 shadow-2xl shadow-gray-200/50 overflow-hidden">
              <div className="bg-gradient-brand px-6 py-4 flex items-center justify-between border-b border-white/10">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-white/30" />
                  <div className="w-3 h-3 rounded-full bg-white/50" />
                  <div className="w-3 h-3 rounded-full bg-white/70" />
                </div>
                <div className="text-[10px] font-mono text-white/80 font-bold tracking-[0.3em] uppercase italic">Diagnostic Core // Live Analysis</div>
              </div>
              <div className="p-6 sm:p-10 md:p-16">
                <div className="flex flex-wrap gap-3 mb-12 border-b border-gray-100 pb-6 justify-center">
                  {tabs.map((tab, i) => (
                    <button
                      key={i}
                      onClick={() => setActiveTab(i)}
                      className={`flex items-center gap-3 px-6 md:px-10 py-4 rounded-2xl text-base font-bold transition-all whitespace-nowrap shrink-0 border-2 ${activeTab === i ? 'bg-gradient-brand border-transparent text-white shadow-xl shadow-orange-500/20 scale-105' : 'bg-white border-gray-100 text-gray-500 hover:border-gray-300 hover:bg-gray-50'}`}
                    >
                      {tab.icon}
                      {tab.label}
                    </button>
                ...
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="grid lg:grid-cols-[1fr_auto_1fr] gap-12 items-center"
                  >
                    <div className="space-y-8">
                      <div>
                        <div className="text-[10px] font-bold text-blue-600 uppercase tracking-[0.2em] mb-2">Diagnostic Result</div>
                        <h3 className="text-3xl font-bold text-slate-900 mb-4">{tabs[activeTab].title}</h3>
                        <p className="text-lg text-gray-600 leading-relaxed font-medium">{tabs[activeTab].data}</p>
                      </div>
                      
                      <div className="space-y-4">
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Key Findings</div>
                        {tabs[activeTab].details.map((detail, idx) => (
                          <div key={idx} className="flex items-start gap-3 bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:border-blue-200 transition-colors">
                            <div className="w-2 h-2 rounded-full bg-gradient-brand mt-1.5 shrink-0" />
                            <p className="text-sm text-slate-700 font-medium leading-tight">{detail}</p>
                          </div>
                        ))}
                      </div>
                    </div>

                    <div className="hidden lg:block w-px h-64 bg-gray-100" />

                    <div className="bg-slate-50 rounded-[2rem] p-8 border border-gray-100 relative overflow-hidden flex flex-col justify-center h-full min-h-[350px]">
                      ...
                      <div className="space-y-10 relative z-10">
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center mb-6">Metrics & Impact</div>
                        <div className="grid grid-cols-1 gap-8">
                          {tabs[activeTab].stats.map((stat, idx) => (
                            <div key={idx} className="text-center group">
                              <div className={`text-4xl md:text-5xl font-black mb-2 transition-transform duration-500 group-hover:scale-110 ${stat.color.includes('slate') ? 'text-slate-900' : ''}`}>
                                {stat.color.includes('text-') && !stat.color.includes('slate') ? (
                                  <span className="text-gradient leading-none">{stat.value}</span>
                                ) : (
                                  stat.value
                                )}
                              </div>
